import { CreateSaveAIBatch, GeneratedBatch, Task } from "@/interfaces/ITaskPlanner";
...
export async function generateTasks(topic: string): Promise<Task[]> {
  try {
    const response = await fetch("/api/ai/taskPlanner/generate-task", {
      method: "POST",
...
export const getAIBatches = async (): Promise<GeneratedBatch[]> => {
  const response = await fetch("/api/ai/taskPlanner/generate-task/ai-batches");
...
export async function saveAITasks(data: { tasks: Task[]; batchId?: string }) {
  const res = await fetch("/api/ai/taskPlanner/save-tasks", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export const deleteAIBatch = async (batchId: string) => {
  const response = await fetch(`/api/ai/taskPlanner/generate-task/ai-batches/${batchId}`, {
    method: "DELETE",
  });
